...
class AbstractGANWithPenalty(AbstractGAN):
    ...
    def __init__(self, model_name, **kwargs):
        ...
        if self.penalty_type not in consts.PENALTIES:
            raise ValueError("Penalty '%s' not recognized." %
                             self.penalty_type)
        ...
        if self.architecture not in consts.ARCHITECTURES:
            raise ValueError("Architecture '%s' not recognized." %
                             self.architecture)
    ...
    def print_progress(self, step, start_step, start_time, d_loss, g_loss,
        ...
        if FLAGS.print_penalty_loss and step % 100 == 0:
            ...
            print("\t\tlambda: %.4f penalty_loss: %.4f" %
                  (self.lambd, penalty_loss))
    ...
    def wgangp_penalty(self, x, x_fake, discriminator, is_training):
        ...
        logits = discriminator(
            interpolates, is_training=is_training, reuse=True)[1]
    ...
    def _model_fn(self, features, labels, mode, params):
        ...
        def create_sub_step_loss(features, sub_step_idx=0, reuse=True):
            ...
            with self._different_batch_size(bs):
                ...
                self.fake_images = self.generator(
                    f["z"], is_training=False, reuse=True)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            with tf.control_dependencies(deps):
                ...
                if self.use_tpu:
                    d_optimizer = tf.contrib.tpu.CrossShardOptimizer(
                        d_optimizer)
                    g_optimizer = tf.contrib.tpu.CrossShardOptimizer(
                        g_optimizer)
    ...
    def build_model(self, is_training=True):
        ...
        with tf.name_scope("discriminator_representation"):
            result, _, _ = self.discriminator(
                tf.placeholder(
                    tf.float32, self.inputs.get_shape(), name="input"),
                is_training=False,
                reuse=True)
            result = tf.identity(result, name="result")
...
class LSGAN_PENALTY(AbstractGANWithPenalty):
    ...
    def __init__(self, **kwargs):
        super(LSGAN_PENALTY, self).__init__(
            consts.LSGAN_WITH_PENALTY, **kwargs)
