...
class EvalGanLibTest(tf.test.TestCase):
    ...
    def test_mock_gan_and_mock_fid(self):
        ...
        workdir = os.path.join(tf.test.get_temp_dir(), self.id())
        tf.logging.info("Workdir: %s" % workdir)
        options = {"gan_type": "GAN",
                   "dataset": "fake", "eval_test_samples": 50}
        ...
        with mock.patch.object(gan_lib, "create_gan", autospec=True) as mock_cls:
            with mock.patch.object(
                ...
                self.assertEquals(
                    result_dict["fid_score"], self._FAKE_FID_SCORE)
            ...
            with tf.Session() as sess:
                ...
                with mock.patch.object(
                    ...
                    mock_cls.return_value = {
                        "inception_score": 12.0, "train_d_loss": 1.3}
                    eval_gan_lib.RunTaskEval(
                        options, workdir, inception_graph=None)

                    rows = self._get_scores(workdir)
                    self.assertEquals(2, len(rows))
                    self.assertNear(
                        float(rows[0]["inception_score"]), 12.0, 0.01)
                    self.assertNear(float(rows[1]["train_d_loss"]), 1.3, 0.01)
                    self.assertNear(
                        float(rows[1]["test_accuracy"]), -1.0, 0.01)

                    # Now create 2 more checkpoints.
                    for x in range(3, 5):
                        saver.save(sess, checkpoint_path, global_step=x)
                    mock_cls.return_value = {
                        "inception_score": 14.0, "train_d_loss": 1.5}
                    eval_gan_lib.RunTaskEval(
                        options, workdir, inception_graph=None)
                    rows = self._get_scores(workdir)
                    ...
                    self.assertNear(
                        float(rows[2]["inception_score"]), 14.0, 0.01)
                    ...
                    with mock.patch.object(
                        ...
                        for x in range(6):
                            self.assertNear(
                                float(rows[x]["inception_score"]), 16.0, 0.01)
                            self.assertNear(
                                float(rows[x]["new_metric"]), 20.0, 0.01)
                            self.assertNear(
                                float(rows[x]["test_accuracy"]), -1.0, 0.01)
                            self.assertNear(
                                float(rows[x]["train_d_loss"]), 1.7, 0.01)
