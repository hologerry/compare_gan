...
def PlotImage(ax, im, title=None, x_label=None, y_label=None, vmin=0.0,
    ...
    if y_label:
        ax.set_ylabel(y_label, rotation=90, fontsize=16)
    ...
    def LatentTraversalFig(aggregated_images_, generated_images_):
        ...
        for i in range(0, (k + 1) * n_images, (k + 1)):
            for t in range(n_steps):
                ...
                for j in range(k):
                    PlotImage(ax[i + j + 1, t],
                              generated_images_[i // (k + 1), t, j])
    ...
    with tf.Graph().as_default() as g:
        with tf.Session() as sess:
            ...
            if (FLAGS.visualization_type == "multi_image" and
                ...
                fake_images, generator_preds = sess.run(
                    fetches, feed_dict=feed_dict)
                SaveMultiGanGeneratorImages(
                    fake_images, generator_preds, out_cp_dir)
            ...
            # Compute outputs for MultiGanLatentTraversalImages
                ...
                for step in range(n_steps + 1):
                    ...
                    images_batch, gen_preds_batch = sess.run(
                        fetches, {gan.z: new_z})
                ...
                SaveMultiGanLatentTraversalImages(
                    images, gen_preds, out_cp_dir)
            ...
            # Compute outputs for GanLatentTraversalImages
                ...
                z_sample = gan.z_generator(gan.batch_size, gan.z_dim)
                directions = np.random.uniform(size=z_sample.shape)
                k_indices = np.random.randint(
                    options.get("k", 1), size=gan.batch_size)
