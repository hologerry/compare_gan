...
class MultiGAN(AbstractGANWithPenalty):
    ...
    def build_model(self, is_training=True):
        ...
        generated = self.generator(
            self.z, is_training=is_training, reuse=False)
    ...
    def attention_block(self, entities, reuse, name="attention_block"):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            values = tf.reshape(
                values, [self.batch_size, k, self.embedding_dim])

            attention_weights = tf.matmul(
                queries, tf.transpose(keys, [0, 2, 1]))
    ...
    def update_z(self, z, reuse):
        ...
        for block in range(self.n_blocks):
            ...
            for head in range(self.n_heads):
                ...
                z_s.append(self.attention_block(
                    z, share_params, name=head_name))
        ...
        # implicit one obtained via thresholding.
            ...
            for i in range(k-1, -1, -1):
                ...
                if self.aggregate == "implicit_alpha":
                    epsilon = 0.1
                    mask = tf.reduce_max(
                        color_a, axis=-1, keep_dims=True) > epsilon
                    alpha_a = tf.where(
    ...
    def generate_images(self, z, is_training, reuse):
        ...
        for i in range(self.k):
            ...
            out_k = super(MultiGAN, self).generator(
                z[:, i], is_training, use_copy)
