...
def ComputeAccuracyLoss(options,
    ...
    for repeat in range(num_repeat):
        ...
        test_predictions = [
            x >= discriminator_threshold for x in test_predictions]
        fake_predictions = [
...
class MultiscaleSSIMTask(EvalTask):
    ...
    def RunAfterSession(self, options, fake_images, real_images):
        ...
        if ShouldRunMultiscaleSSIM(options):
            result_dict[self._MS_SSIM] = ComputeMultiscaleSSIMScore(
                fake_images)
            logging.info("MS-SSIM score computed: %.3f",
                         result_dict[self._MS_SSIM])
        return result_dict
...
def RunCheckpointEval(checkpoint_path, task_workdir, options, tasks_to_run):
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            for task in tasks_to_run:
                result_dict.update(task.RunInSession(
                    options, sess, gan, real_images))
    ...
    for task in tasks_to_run:
        result_dict.update(task.RunAfterSession(
            options, fake_images, real_images))
    ...
    with tf.gfile.Open(scores_path, "a") as f:
        ...
        for checkpoint_path in all_checkpoint_paths:
            if checkpoint_path in finished_checkpoints:
                logging.info("Skipping already computed path: %s",
                             checkpoint_path)
                continue
            ...
            for task_metric in task_headers:
                output_row.append("%.3f" % result_dict.get(
                    task_metric, default_value))
